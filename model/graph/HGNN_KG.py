...
from util.sampler import next_batch_pairwise, next_batch_kg 
import torch
import torch.nn as nn 
import torch.nn.functional as F
...
class HGNN(GraphRecommender):
    ...
    def train(self):
        ...
        for ep in range(self.maxEpoch):
            ...
            for  batch in tqdm(next_batch_kg(self.data, self.batchSize)):
                h_idx, r_idx, pos_t_idx, neg_t_idx = batch
                model.train()
                # s_model = time.time()
                h_emb, r_emb, t_emb = model(mode='kg')

                h_emb, r_emb, t_emb = rec_user_emb[user_idx], rec_item_emb[pos_idx], rec_item_emb[neg_idx]
                rec_loss, reg_loss = self.calculate_loss(user_emb, pos_item_emb, neg_item_emb, self.batchSize)
                # kg_loss, reg_kg_loss = calculate_kg_loss(anchor_i_emb, ent_emb, ent_emb_neg, batchSize, reg)
                # ssl_loss = calculate_ssl_loss()
                cl_loss = self.cl_rate * model.cal_cl_loss([user_idx, pos_idx], dropped_adj1,dropped_adj2)
...
class HGNNModel(nn.Module):
    def __init__(self, config, data):
        ...
        self.hgnn_e = [ HGNNConv(leaky=self.p, input_dim=self.hyper_dim, hyper_dim=self.hyper_dim) for _ in range(self.n_layers)]

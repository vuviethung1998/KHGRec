...
class Knowledge2(InteractionKG):
...
class Knowledge(Interaction):
    ...
    def construct_data(self):
        ...
        for idx, u in enumerate(lst_user_entities):
            self.u2id[u] = idx
            self.cfuid2kguid[self.user[u]] = idx
            self.kguid2cfuid[idx] = self.user[u] 

        for idx, i in enumerate(lst_item_entities):
            self.i2id[i] = idx
            self.cfiid2kgiid[self.item[i]] = idx
            self.kgiid2cfiid[idx] = self.item[i]

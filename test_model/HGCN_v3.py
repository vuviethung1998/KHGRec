...
class HGCN_Encoder(nn.Module):
    def __init__(self, data, emb_size, hyper_size, n_layers, leaky, drop_rate, device):
        ...
        self.hgnn_layers = []
        self.res_fc = nn.Linear(emb_size, hyper_size).to(device)
    ...
    def forward(self, keep_rate=1):
        ...
        residual = self.res_fc(ego_embeddings)
        all_embeddings = []
...
class HGCNConv(nn.Module):
    ...
    def forward(self, adj, embs, act=True):
        if act:
            return self.act(torch.sparse.mm(torch.sparse.mm(adj, torch.sparse.mm(adj.t(), embs)), self.W)) 
        else:

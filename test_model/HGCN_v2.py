    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                rec_user_emb, rec_item_emb = model(keep_rate=0.5)
...
class HGCN_Encoder(nn.Module):
    def __init__(self, data, emb_size, hyper_size, n_layers, leaky, drop_rate, device):
        ...
        for i in range(self.layers):
            if i ==0:
                self.hgnn_layers.append(HGCNConv(leaky=leaky, input_dim=self.latent_size, hyper_dim=hyper_size, device=device))
            else:
                self.hgnn_layers.append(HGCNConv(leaky=leaky, input_dim=self.hyper_size, hyper_dim=hyper_size, device=device))
    ...
    def forward(self, keep_rate=1):
        ...
        for k in range(self.layers):
            if k != self.layers - 1: 
                hyperLat = self.hgnn_layers[k](sparse_norm_adj, ego_embeddings)
            else:
                hyperLat = self.hgnn_layers[k](sparse_norm_adj, ego_embeddings, act=False)
            if k!= 0:
                ego_embeddings += hyperLat
            else:
                ego_embeddings = hyperLat
...
class HGCNConv(nn.Module):
    def __init__(self, leaky, input_dim, hyper_dim, device):
        ...
        self.res_fc = nn.Linear(emb_size, hyper_size).cuda()
        self.act = nn.LeakyReLU(negative_slope=leaky)
        self.W= nn.Parameter(initializer(torch.empty(input_dim, hyper_dim))).to(device)
        self.bias = nn.Parameter(torch.zeros(hyper_dim),requires_grad=True).to(device)
    
    def forward(self, adj, embs, act=True):
        residual = self.res_fc(embs)
        if act:
            return self.act(torch.sparse.mm(torch.sparse.mm(adj, torch.sparse.mm(adj.t(), embs)), self.W)) + residual
        else:
            return torch.sparse.mm(torch.sparse.mm(adj, torch.sparse.mm(adj.t(), embs)), self.W) + residual
